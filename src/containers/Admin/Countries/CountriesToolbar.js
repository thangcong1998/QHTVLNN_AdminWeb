...
import CountriesForm from "./CountriesFrom";
...
const CountriesToolbar = React.memo((props) => {
  ...
  const addCategory = async () => {
    await dialog({
      title: "Thêm quốc gia",
      content: <CountriesForm close={handleClose} refetch={refetch} />,
    });
  };
  ...
  const deleteChecked = async () => {
    ...
    try {
      const res = await api.fetcher("post", "admin/destroyCountries", {
        list: checked,
  ...
  const exportData = async () => {
    try {
      ...
      if (res) {
        ...
        link.href = url;
        link.setAttribute("download", "Quốc gia.xlsx");
