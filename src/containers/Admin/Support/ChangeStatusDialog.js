...
const ChangeStatus = React.memo((props) => {
    const { status, handleClose, id, refetch, type,formData} = props;
    const api = useAPI()
    ...
    const ChangeStatusRequest = async () => {
        try {
            const res = await api.fetcher('put', '/admin/changeStatusSchoolFeeSupport/' + id, { status: status, note: formik.values?.note, formData });
            handleClose();
            refetch();
        } catch (e) {
