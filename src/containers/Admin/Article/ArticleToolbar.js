...
const ArticleToolbar = React.memo((props) => {
  const { setParams, checked, refetch, trashed, columns, ...others } = props;
  const classes = useStyles();
  const history = useHistory();
  const { dialog, handleClose } = useDialog();
  const api = useAPI();
  const deleteChecked = async (action) => {
    await dialog({
      title: action === "delete" ? "Xóa các hàng đã chọn ?" : "Khôi phục các hàng đã chọn ?",
      type: "confirm",
      confirmationText: "Xác nhận",
      cancellationText: "Bỏ qua",
    });
    try {
      const res = await api.fetcher('delete', 'admin/bulkDestroyArticle', { ids: checked, action: action })
      if (res) {
        refetch();
      }
    } catch {
  ...
  const exportData = async () => {
    try {
      ...
      if (res) {
        ...
        link.href = url;
        link.setAttribute("download", "Bài viết.xlsx");
  ...
  const TrashMode = () => {
    return checked.length > 0 ? (
      <>
        ...
        <div>
          <IconButton title="Xóa" onClick={() => deleteChecked('force')}>
            <DeleteForeverIcon />
          </IconButton>
          <IconButton
            title="Khôi phục"
            onClick={() => deleteChecked('restore')}
  ...
  const NormalMode = () => {
    return checked.length > 0 ? (
      <>
        ...
        <div>
          <IconButton title="Xóa" onClick={() => deleteChecked('delete')}>
            <DeleteIcon />
          </IconButton>
        </div>
      </>
