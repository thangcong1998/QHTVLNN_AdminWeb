...
const CompanyTypesToolbar = React.memo((props) => {
  ...
  const addCategory = async () => {
    await dialog({
      ...
      content: <CompanyTypesForm close={handleClose} refetch={refetch} />,
    });
  };
  ...
  const deleteChecked = async () => {
    ...
    try {
      const res = await api.fetcher("post", "admin/destroyCompanyTypes", {
        list: checked,
  ...
  const exportData = async () => {
    try {
      ...
      if (res) {
        ...
        link.href = url;
        link.setAttribute("download", "Loại công ty.xlsx");
