...
const laborToolBar = React.memo((props) => {
  const {
    setParams,
    setChecked,
    checked,
  ...
  const exportData = async () => {
    try {
      ...
      if (res) {
        ...
        link.href = url;
        link.setAttribute("download", "Sở LĐ-XH.xlsx");
  ...
  const deleteChecked = async () => {
    ...
    try {
      const res = await api.fetcher(
        "post",
        "admin/destroyListLaborDepartment",
        {
          list: checked,
