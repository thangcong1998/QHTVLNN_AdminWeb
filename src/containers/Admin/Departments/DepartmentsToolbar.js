...
const DepartmentsToolbar = React.memo((props) => {
  ...
  const addCategory = async () => {
    await dialog({
      ...
      content: <DepartmentsForm close={handleClose} refetch={refetch} />,
    });
  };
  ...
  const deleteChecked = async () => {
    ...
    try {
      const res = await api.fetcher("post", "admin/destroyDepartments", {
        list: checked,
  ...
  const exportData = async () => {
    try {
      ...
      if (res) {
        ...
        link.href = url;
        link.setAttribute("download", "Phòng ban.xlsx");
