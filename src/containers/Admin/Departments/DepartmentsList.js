...
const DepartmentsList = React.memo((props) => {
  ...
  const deleteConfirm = async (row) => {
    ...
    try {
      const res = await api.fetcher("delete", "admin/departments/" + row.id, {
  ...
  const editCategory = async (row) => {
    await dialog({
      ...
      content: (
        <DepartmentsForm row={row} close={handleClose} refetch={refetch} />
      ),
    });
  };
  return (
