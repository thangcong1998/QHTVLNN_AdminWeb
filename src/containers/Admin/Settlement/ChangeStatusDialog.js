...
const ChangeStatus = React.memo((props) => {
    ...
    const handleSettlement = async () => {
        try {
            let res = await api.fetcher('put', 'admin/changeStatusSettlement/' + id, { status: status, note: formik?.values.note });
            handleClose();
            refetch();
        } catch (e) {
        }
    ...
    return (
        <div>
            ...
            <Button color="primary" variant="contained" onClick={handleSettlement} >Xác nhận</Button>
        </div>
    )
})
